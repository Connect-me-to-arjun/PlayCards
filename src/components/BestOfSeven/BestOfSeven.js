...
import './BestOfSeven.scss';
class BestOfSeven extends Component {
    state = {
        currentRound: 0,
        totalRounds: 7,
        currentOpenFor: 0,
    ...
    generateRows = () => {
        ...
        for (let i = 0; i <= this.state.currentRound; i++) {
            ...
            >
                ...
                {playersList.map((player, index) => <TableCell key={index + i} className={i === this.state.currentRound && index === currentOpenFor ? 'currentOpen' : ''} onClick={() => { this.handleDialog(true, playersList[index], i + 1) }} align="center">{this.state.playerScores[player][i] > -1 ? this.state.playerScores[player][i] : "+"}</TableCell>)}
            </TableRow>)
        }
        return arr
    }
    ...
    componentDidMount() {
        const { data: { names, selected } } = this.props;
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevState.currentRound !== this.state.currentRound) {
            if (this.props.data.selected.length === this.state.currentRound) {
                this.setState({
                    currentOpenFor: 0
                })
            }
    ...
    render() {
        const { currentRound, playersList, currentPersonForScore, forRound } = this.state
        return (
            <div>
                    ...
                    <Table sx={{ minWidth: 650 }} aria-label="simple table">
                        <TableHead>
                            <TableRow>
                                ...
                                {playersList.map((name, index) => <TableCell key={index + name} align="center" >{name}</TableCell>)}
                            </TableRow>
                        </TableHead>
                        <TableBody>
                            ...
                            <TableRow>
                                ...
                                {playersList.map((name, index) => <TableCell key={index + name} align="center" >{this.getTotalScore(name)}</TableCell>)}
                            </TableRow>
                        </TableBody>
                    </Table>
                </TableContainer>
                <div className="completeButton">
                    ...
                    <Button className='MediumButton clearAll' variant="contained" color="primary" size="large" endIcon={<NavigateNextIcon />} onClick={this.startNewRound}>Start New Round</Button>
                    </div>
                    
                </div>
                <AddscoreDialog round={forRound} personName={currentPersonForScore} open={this.state.openDialog} handle={this.handleDialog} set={this.setScore} />
            </div>
        );
    }
}
